"""
AMD Scenario 6: visible-device index mapping.

HIP_VISIBLE_DEVICES selects a subset of GPUs.  gpuq must map global
(system-wide) indices to local (visible) indices correctly.

E.g. HIP_VISIBLE_DEVICES=4,5,6 on an 8-GPU system:
...
Run with: HIP_VISIBLE_DEVICES=4,5,6 python e2e/amd_06_index_mapping.py
"""
...
VISIBLE_IDS = [int(x) for x in os.environ["HIP_VISIBLE_DEVICES"].split(",")]
print(
    f"System has {TOTAL} AMD GPU(s), HIP_VISIBLE_DEVICES={os.environ['HIP_VISIBLE_DEVICES']}"
)
...
for local_idx, gpu in enumerate(visible):
    ...
    assert gpu.is_visible, f"visible[{local_idx}].is_visible: expected True"
