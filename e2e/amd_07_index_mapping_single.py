"""
AMD Scenario 7: single visible GPU — index mapping sanity check.

HIP_VISIBLE_DEVICES=0 on a multi-GPU system: only GPU 0 is visible,
local index 0 maps to system_index 0.

Run with: HIP_VISIBLE_DEVICES=0 python e2e/amd_07_index_mapping_single.py
"""
...
print(f"System has {TOTAL} AMD GPU(s), HIP_VISIBLE_DEVICES=0")
...
assert visible[0].system_index == 0, f"visible[0].system_index: expected 0, got {visible[0].system_index}"
...
for gpu in all_gpus:
    if gpu.system_index == 0:
        ...
        assert gpu.index is None, f"system_index={gpu.system_index}: expected None, got {gpu.index}"
